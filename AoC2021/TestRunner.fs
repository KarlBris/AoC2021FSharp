...
module TestRunner =
    ...
    let getInput day = 
        ...
        if File.Exists filename then File.ReadAllText filename else failwith $"Input file input_day{day}.txt not found"

    let makeComparison (expectedResults: string list) (results: string list) =
        List.zip results expectedResults |> List.map (fun(r,e) -> (r,e,r=e))
    ...
    let runExamplesAndMain examples expectedResults realInput (func: string -> string) title =
        let resultList = examples |> List.map func |> makeComparison expectedResults
        printfn title
        ...
        resultList |> List.map printStatus |> ignore
        let examplesSuccessful = resultList |> List.fold (fun b1 (_,_,b2) -> b1 && b2) true
